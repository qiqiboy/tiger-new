module.exports = {
    ...
    husky: {
        hooks: {
            ...
            'pre-push':
                'CF=$(git diff --diff-filter AM --name-only @{u}..) || CF=$(git diff --diff-filter AM --name-only origin/master...HEAD); FILES=$(echo "$CF" | grep -E \'^app/.*\\.[jt]sx?$\'); echo $FILES; if [ -n "$FILES" ]; then node_modules/.bin/eslint $FILES --max-warnings 0; fi'
        }
    },
    eslintConfig: {
