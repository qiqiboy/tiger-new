...
export const open = (settings = {}) => {
    ...
    let elements = createElementFromString(
        (settings.backdrop ? `<div class="modal-backdrop" style="z-index:${zIndex};"></div>` : '') +
            `<div style="z-index:${zIndex};" class="modal${
                settings.windowTopClass ? ' ' + settings.windowTopClass : ''
            }"></div>`
    );
    ...
    function render(component, onComplete) {
        ...
        return reactRender(
            <ModalContainer modalElements={elements} {...settings} close={close} dismiss={dismiss} />,
            modalElement,
            onComplete
        );
    }
