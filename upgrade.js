...
function upgradeAppProject(root) {
    ...
    var husky = path.resolve(root, '.husky');
    ...
    var pickDependencies = function (keys, type) {
        const dependencies = newDependenciesConfig[type || 'dependencies'] || {};

        return keys.map(function (key) {
            return !dependencies[key] || dependencies[key] === 'latest' ? key : key + '@' + dependencies[key];
        });
    };
        ...
        .then(answers => {
            if (answers.upgrade) {
                ...
                inquirer.prompt(questions).then(answers => {
                    ...
                    var tsconfigJson = fs.readJsonSync(tsconfig);
                    var tscUpdated = false;

                    ['suppressImplicitAnyIndexErrors', 'importHelpers'].forEach(option => {
                        if (tsconfigJson.compilerOptions[option]) {
                            delete tsconfigJson.compilerOptions[option];
                            tscUpdated = true;
                        }
                    });
                    ...
                    if (package.scripts.tsc) {
                        delete package.scripts.tsc;
                    }

                    if (package.husky || answers.updateHusky) {
                        delete package.husky;

                        fs.copySync(path.resolve(ownPath, 'template/application/husky'), husky, {
                            overwrite: true
                        });

                        spinner.succeed(chalk.green('已升级 husky 到v9 ！'));
                    }
                    ...
                    Promise.all([
                        install(
                            pickDependencies(
                                _.difference(Object.keys(newDependenciesConfig.devDependencies), [
                                    '@types/react',
                                    '@types/react-dom'
                                ]),
                                'devDependencies'
                        ...
                        .then(function () {
                            if (answers.upgradeReact) {
                                return Promise.all([
                                    install(
                                        pickDependencies(['react', 'react-dom', 'react-formutil'], 'dependencies'),
                                        false
                                    ),
                                    install(
                                        pickDependencies(['@types/react', '@types/react-dom', 'devDependencies']),
                                        true
                                    )
                                ]).then(function () {
                                    console.log();
                        ...
                        .then(function () {
                            try {
                                ...
                                if (semver.lt(reactVersion, '19.0.0')) {
                                    return install(
                                        pickDependencies(['react-compiler-runtime'], 'extraDependencies'),
                                        false,
                                        function () {
                                            console.log();
